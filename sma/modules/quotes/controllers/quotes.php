...
//index or inventories page
   ...
   function getquotes()
   {
	   ...
	   $this->datatables
			...
			->from('quotes')
			->add_column("Actions", 
			"<center><a href='#' title='$2' class='tip' data-html='true'><i class='icon-folder-close'></i></a> <a href='#' onClick=\"MyWindow=window.open('index.php?module=quotes&view=view_quote&id=$1', 'MyWindow','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=1000,height=600'); return false;\" title='".$this->lang->line("view_quote")."' class='tip'><i class='icon-fullscreen'></i></a> 
...
//view inventory as html page
   ...
   function view_quote()
   {
	   if($this->input->get('id')){ $id = $this->input->get('id'); } else { $id = NULL; }
		   ...
		   $data['rows'] = $this->quotes_model->getAllQuoteItems($id);
