...
<?php if($message) { echo "<div class=\"alert alert-error\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\">&times;</button>" . $message . "</div>"; } ?>
	...
	<?php $attrib = array('class' => 'form-horizontal', 'id' => 'addSale_form'); echo form_open("module=transfers&view=edit&id=".$id, $attrib); ?>
  ...
  <div class="controls">  <?php 
	 $wh[''] = '';
	  foreach($warehouses as $warehouse){
		...
		echo form_dropdown('from_warehouse', $wh, (isset($_POST['warehouse']) ? $_POST['warehouse'] : $transfer->from_warehouse_id), 'id="fwarehouse_s" data-placeholder="'.$this->lang->line("select").' '.$this->lang->line("warehouse").' ('.$this->lang->line("from").')" required="required" data-error="'.$this->lang->line("warehouse").' ('.$this->lang->line("from").') '.$this->lang->line("is_required").'"'); ?> </div>
</div>
<div class="control-group">
  <label class="control-label" id="twarehouse_l"><?php echo $this->lang->line("to"); ?></label>
  <div class="controls">  <?php 
		echo form_dropdown('to_warehouse', $wh, (isset($_POST['warehouse']) ? $_POST['warehouse'] : $transfer->to_warehouse_id), 'id="twarehouse_s" data-placeholder="'.$this->lang->line("select").' '.$this->lang->line("warehouse").' ('.$this->lang->line("to").')" required="required" data-error="'.$this->lang->line("warehouse").' ('.$this->lang->line("to").') '.$this->lang->line("is_required").'"'); ?> </div>
</div>
...
<table id="dyTable" class="table items table-striped table-bordered table-condensed table-hover">
  ...
  <tbody>
  	 <?php $r = 1; foreach($transfer_items as $prod){ 
			
             echo '<tr id="row_'.$r.'"><td><input name="product'.$r.'" type="hidden" value="'.$prod->product_code.'"><input class="span5 tran" style="text-align:left;" name="item'.$r.'" type="text" value="'. $prod->product_name .' ('. $prod->product_code .')"></td><td><input class="span2 tran" name="quantity'.$r.'" type="text" value="'.$prod->quantity.'" onClick="this.select();"></td><td><i class="icon-trash tip del" id="'.$r.'" title="Remove this Item" style="cursor:pointer;" data-placement="right"></i></td></tr>'; 
		$r++; } ?>
  </tbody>
</table>
	</div>
